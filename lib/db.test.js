...
//
// Cleanup: if db/resume.db did NOT exist before the test started (i.e. the test
// created it), we delete it afterward so the working tree stays clean.
// If it already existed (dev is running the app locally), we leave it alone.
...
// Snapshot existence BEFORE the import so the after() hook knows whether to clean up
const boolDbExistedBefore = existsSync(strDbPath);
...
// ── Cleanup ────────────────────────────────────────────────────────────────
after(() => {
    // Close the connection before touching the file — required on WSL/Windows
    // where open file handles block deletion
    if (dbDatabase && typeof dbDatabase.close === "function") {
        dbDatabase.close();
    }

    // Only remove the DB if this test run created it
    if (!boolDbExistedBefore && existsSync(strDbPath)) {
        unlinkSync(strDbPath);
    }
});
