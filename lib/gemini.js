// Google Gemini API wrapper; exports reviewSection(sectionType, text) returning suggestion strings.
...
const REVIEW_RULES = "You are a professional resume reviewer. Only report genuine issues. If the content is already strong, return []. When real issues exist, state them directly and confidently. Return ONLY a JSON array of suggestion strings (or [] if none), no other text.";

// Per-section focus instructions appended after the shared rules.
const SECTION_FOCUS = {
    summary: "Focus on: leading with a clear professional identity and years of experience; including key skills and domains relevant to target roles; conveying measurable impact where possible (e.g. '5+ years building scalable APIs'); keeping to 2–4 sentences; eliminating vague filler like 'results-driven' or 'team player'.",
    ...
    projects: "Focus on: stating the problem solved and the outcome achieved; naming specific technologies used (e.g. 'Built with React, Node.js, and PostgreSQL'); following the pattern what it does + how you built it + result or impact; including metrics where available (e.g. user count, performance gains, uptime).",
    education: "Focus on: verifying degree, field, institution, and dates are all present; suggesting inclusion of GPA if 3.5 or above; noting relevant coursework, honors, or academic achievements if missing; flagging inconsistent date formatting.",
    ...
    certifications: "Focus on: verifying the issuer name and date earned or expiry are present; flagging abbreviated or unclear certification names (e.g. prefer 'AWS Certified Solutions Architect — Associate' over 'AWS SAA'); suggesting prioritization of certifications most relevant to the target role.",
...
const OUTPUT_LEAKAGE_REGEX = /system\s+prompt|my\s+instructions?|you\s+are\s+now|ignore\s+previous/i;
...
function assertResponseClean(strResponse) {
    if (OUTPUT_LEAKAGE_REGEX.test(strResponse)) {
        console.warn("lib/gemini.js — response flagged for potential prompt leakage; discarding");
...
export async function reviewSection(sectionType, text) {
    ...
    const strCleaned = strResponse.replace(/```json\n?/g, "").replace(/```\n?/g, "").trim();
