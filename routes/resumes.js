...
router.put("/:id", (req, res) => {
    ...
    try {
        const result = db
            .prepare(
                "UPDATE resumes SET name = @name, target_role = @target_role WHERE id = @id",
            )
...
router.delete("/:id", (req, res) => {
    ...
    try {
        const result = db
            .prepare("DELETE FROM resumes WHERE id = @id")
            .run({ id });
        if (result.changes === 0) {
            return res.status(404).json({ message: "Resume not found" });
        }
...
router.put("/:id/selections", (req, res) => {
    ...
    if (
        ...
        return res
            .status(400)
            .json({ message: "sections, items, and bullets must be arrays" });
    }

    try {
        // Verify resume exists before wiping any data
        const arrResume = db
            .prepare("SELECT id FROM resumes WHERE id = @id")
            .all({ id: resume_id });
        ...
        for (const s of sections) {
            if (!s || typeof s !== "object") {
                return res
                    .status(400)
                    .json({ message: "section entries must be objects" });
            }
            if (!SECTION_TYPES.has(s.section_type)) {
                return res.status(400).json({
                    message: `Invalid section_type: ${s.section_type}`,
                });
            }
            if (![0, 1].includes(s.included ?? 1)) {
                return res
                    .status(400)
                    .json({ message: `Invalid included value: ${s.included}` });
            }
        }
        ...
        for (const item of items) {
            if (!item || typeof item !== "object") {
                return res
                    .status(400)
                    .json({ message: "items entries must be objects" });
            }
            const strTable = SECTION_ITEM_TABLES[item.section_type];
            if (!strTable) {
                return res.status(400).json({
                    message: `Invalid section_type: ${item.section_type}`,
                });
            }
            const arrItem = db
                .prepare(`SELECT id FROM ${strTable} WHERE id = @id`)
                .all({ id: item.item_id });
            if (arrItem.length === 0) {
                return res.status(404).json({
                    message: `Item ${item.item_id} not found in ${item.section_type}`,
                });
            }
        }
        ...
        for (const b of bullets) {
            if (!b || typeof b !== "object") {
                return res
                    .status(400)
                    .json({ message: "bullets entries must be objects" });
            }
            const strBulletTable =
                b.bullet_type === "job"
                    ? "job_bullets"
            ...
            if (!strBulletTable) {
                return res
                    .status(400)
                    .json({ message: `Invalid bullet_type: ${b.bullet_type}` });
            }
            // bullet_type "job" → items section_type "jobs"; "project" → "projects"
            const strParentSectionType =
                b.bullet_type === "job" ? "jobs" : "projects";
            if (
                !setSelectedItems.has(
                    `${strParentSectionType}:${b.parent_item_id}`,
                )
            ) {
                return res.status(400).json({
            ...
            const strParentCol =
                b.bullet_type === "job" ? "job_id" : "project_id";
            const arrBullet = db
                .prepare(
                    `SELECT id FROM ${strBulletTable} WHERE id = @id AND ${strParentCol} = @parent_id`,
                )
        ...
        const saveSelections = db.transaction(() => {
            ...
            db.prepare(
                "DELETE FROM resume_sections WHERE resume_id = @resume_id",
            ).run({ resume_id });
            db.prepare(
                "DELETE FROM resume_items WHERE resume_id = @resume_id",
            ).run({ resume_id });
            db.prepare(
...
router.get("/:id/selections", (req, res) => {
    ...
    try {
        ...
        const arrSections = db
            .prepare(
                "SELECT section_type, included, sort_order FROM resume_sections WHERE resume_id = @resume_id ORDER BY sort_order",
            )
            .all({ resume_id });

        const arrItems = db
            .prepare(
                "SELECT section_type, item_id, sort_order FROM resume_items WHERE resume_id = @resume_id ORDER BY sort_order",
            )
            .all({ resume_id });

        const arrBullets = db
            .prepare(
                "SELECT parent_item_id, bullet_type, bullet_id, sort_order FROM resume_bullets WHERE resume_id = @resume_id ORDER BY sort_order",
            )
            .all({ resume_id });
