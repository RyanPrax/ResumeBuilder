...
router.delete("/:id", (req, res) => {
    ...
    try {
        ...
        const deleteCertification = db.transaction(() => {
            db.prepare("DELETE FROM resume_items WHERE section_type = 'certifications' AND item_id = @id").run({ id });
            return db.prepare("DELETE FROM certifications WHERE id = @id").run({ id });
        });
