...
after(async () => {
    ...
    if (intTestId !== null) {
        db.prepare("DELETE FROM educations WHERE id = @id").run({
            id: intTestId,
        });
    }
});
...
test("DELETE /api/educations/:id — removes orphaned resume_items rows", async () => {
    ...
    const objEd = db
        .prepare("INSERT INTO educations (institution) VALUES (@institution)")
        .run({ institution: "Orphan Test University" });
    ...
    const arrOrphans = db
        .prepare(
            "SELECT id FROM resume_items WHERE section_type = 'education' AND item_id = @item_id",
        )
        .all({ item_id: intEdId });
    assert.equal(
        arrOrphans.length,
        0,
        "resume_items row must be removed when education is deleted",
    );
