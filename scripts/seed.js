...
const stmtInsertResume = db.prepare(
    "INSERT INTO resumes (name, target_role) VALUES (?, ?)",
);
const stmtInsertSection = db.prepare(
    "INSERT INTO resume_sections (resume_id, section_type, included, sort_order) VALUES (?, ?, 1, ?)",
);
const stmtInsertItem = db.prepare(
    "INSERT INTO resume_items (resume_id, section_type, item_id, sort_order) VALUES (?, ?, ?, ?)",
);
const stmtInsertBulletSel = db.prepare(
    "INSERT INTO resume_bullets (resume_id, parent_item_id, bullet_type, bullet_id, sort_order) VALUES (?, ?, ?, ?, ?)",
);
const stmtInsertSummary = db.prepare(
    "INSERT INTO summaries (label, content) VALUES (?, ?)",
);
const stmtInsertEdu = db.prepare(
    "INSERT INTO educations (institution, degree, field, end_date, gpa) VALUES (?, ?, ?, ?, ?)",
);
const stmtInsertJob = db.prepare(
    "INSERT INTO jobs (company, title, location, start_date, end_date, is_current, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)",
);
const stmtInsertJobBullet = db.prepare(
    "INSERT INTO job_bullets (job_id, text, sort_order) VALUES (?, ?, ?)",
);
const stmtInsertProject = db.prepare(
    "INSERT INTO projects (name, description, start_date, end_date, is_current, sort_order) VALUES (?, ?, ?, ?, ?, ?)",
);
const stmtInsertProjBullet = db.prepare(
...
function insertProject(
    ...
    const bulletIds = bullets.map((text, i) => {
        const { lastInsertRowid } = stmtInsertProjBullet.run(
            projectId,
            text,
            i,
        );
...
if (stmtResumeExists.get(BEGINNER_NAME)) {
    ...
    db.transaction(() => {
        ...
        // Skills
        const { skillIds: langIds } = insertSkillCategory("Languages", 0, [
            "Python",
            "JavaScript",
            "Java",
            "SQL",
        ]);
        const { skillIds: webIds } = insertSkillCategory("Web", 1, [
            "React",
            "Node.js",
            "Express",
            "HTML/CSS",
        ]);
        const { skillIds: toolIds } = insertSkillCategory("Tools", 2, [
            "Git",
            "Linux",
            "PostgreSQL",
            "Docker",
        ...
        // Resume + wiring
        const { lastInsertRowid: resumeId } = stmtInsertResume.run(
            BEGINNER_NAME,
            "Junior Software Engineer",
        );

        insertSections(resumeId, [
            "contact",
            "summary",
            "skills",
            "jobs",
...
if (stmtResumeExists.get(MID_NAME)) {
    ...
    db.transaction(() => {
        ...
        const { skillIds: webIds } = insertSkillCategory("Web & APIs", 1, [
            "React",
            "gRPC",
            "GraphQL",
            "REST",
        ]);
        const { skillIds: dataIds } = insertSkillCategory("Data", 2, [
            "PostgreSQL",
            "Redis",
            "Kafka",
            "Elasticsearch",
        ]);
        const { skillIds: cloudIds } = insertSkillCategory(
            "Cloud & DevOps",
            3,
...
if (stmtResumeExists.get(SENIOR_NAME)) {
    ...
    db.transaction(() => {
        ...
        const { skillIds: cloudIds } = insertSkillCategory("Cloud & Infra", 3, [
            "AWS",
            "GCP",
            "Kubernetes",
            "Terraform",
